...
public:
    ...
    Span(const std::vector<T>& vec) : m_data(const_cast<T*>(vec.data())), m_size(vec.size()) {}
...
public:
    ...
    Span(const std::vector<T>& vec) : m_data(vec.data()), m_size(vec.size()) {}
    
    const T* data() const { return m_data; }
    size_t size() const { return m_size; }
    
    Span<const T> last(size_t count) const {
        return Span<const T>(m_data + m_size - count, count);
    }
    
    Span<const T> subspan(size_t offset, size_t count) const {
        return Span<const T>(m_data + offset, count);
    }
...
template<typename T>
Span<T> MakeSpan(const T* data, size_t size) {
    return Span<T>(data, size);
}

template<typename T>
Span<T> MakeSpan(std::vector<T>& vec) {
    return Span<T>(vec);
}

template<typename T>
Span<T> MakeSpan(const std::vector<T>& vec) {
    return Span<T>(vec);
}

template<typename T, size_t N>
Span<T> MakeSpan(prevector<N, T>& vec) {
    return Span<T>(vec.data(), vec.size());
}

template<typename T, size_t N>
Span<T> MakeSpan(const prevector<N, T>& vec) {
    return Span<T>(const_cast<T*>(vec.data()), vec.size());
}
...
class CNetAddr
{
    ...
    public:
        ...
        std::vector<unsigned char> GetGroup(const std::vector<bool> &asmap = std::vector<bool>()) const;
